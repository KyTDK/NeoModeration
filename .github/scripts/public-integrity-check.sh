...
# Allow .gitignore to list ignored tooling directories without triggering the gate;
# same for the hygiene test which constructs forbidden strings at runtime.
if git grep -I -q -i -E -- "$pattern" "$head_sha" -- . ':!.gitignore' ':!src/test/java/com/neomechanical/neomoderation/ProductionHygieneTest.java' ; then
